...
function CurrentPerformer({ perf }: { perf: PerformanceWithDetails }) {
  return (
    <div className="text-center">
      <div className="text-navy-400 text-lg sm:text-xl mb-3 uppercase tracking-wider">
        現在の演技
      </div>
      <div className="text-4xl sm:text-6xl font-bold text-white mb-2">
        {perf.athlete.name}
      </div>
      <div className="text-navy-400 text-xl sm:text-2xl mb-6">
      ...
      {perf.dScore !== null && (
        <div className="inline-block bg-navy-800 rounded-2xl px-10 py-6">
          ...
          <div className="font-mono text-5xl sm:text-7xl font-bold text-accent-light">
            {perf.dScore.toFixed(1)}
          </div>
        </div>
      )}
      <div className="mt-6 text-navy-500 text-lg animate-live-pulse">
        採点中...
      </div>
    </div>
  );
...
function LatestResult({ perf }: { perf: PerformanceWithDetails }) {
  return (
    <div className="text-center">
      ...
      <div className="text-navy-400 text-lg sm:text-xl mb-6">
        {perf.athlete.affiliation}
      </div>
      <div className="font-mono text-7xl sm:text-9xl font-bold text-white mb-6">
      ...
      {perf.rank && (
        <div className="mt-6 text-3xl text-gold font-bold">
          現在 {perf.rank}位
        </div>
      )}
    </div>
...
function RankingView({ performances }: { performances: PerformanceWithDetails[] }) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <div className="text-navy-400 text-center text-lg mb-4 uppercase tracking-wider">
        全体順位
      </div>
          ...
          <div
            ...
            className={`flex items-center gap-4 px-6 py-3 rounded-xl ${
              p.rank === 1
                ...
                : p.rank === 2
                ? "bg-silver/20 border border-silver/30"
                : p.rank === 3
                ? "bg-bronze/20 border border-bronze/30"
                : "bg-navy-800/50"
            }`}
          >
            <div className="w-12 text-center">
              {p.rank! <= 3 ? (
                <Trophy className={`w-6 h-6 mx-auto ${
                  p.rank === 1 ? "text-gold" : p.rank === 2 ? "text-silver" : "text-bronze"
                }`} />
              ) : (
                <span className="text-2xl font-bold text-navy-400">{p.rank}</span>
              )}
            </div>
...
export default function ScoreboardPage({
  ...
  const confirmedPerfs = performances
    .filter((p) => p.status === "confirmed" && p.rank !== null)
    .sort((a, b) => a.rank! - b.rank!);
  const latestConfirmed = [...confirmedPerfs].sort(
    (a, b) => (b.finalScore ?? 0) - (a.finalScore ?? 0)
  ).find((p) => {
    // Find the most recently confirmed (highest startOrder among confirmed)
    const allConfirmed = performances.filter((x) => x.status === "confirmed");
    return p === allConfirmed.sort((x, y) => y.athlete.startOrder - x.athlete.startOrder)[0];
  }) ?? confirmedPerfs[confirmedPerfs.length - 1];

  // Show current performer > latest result > ranking
    ...
    <div className="min-h-screen bg-navy-900 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        {currentPerf ? (
          ...
          <RankingView performances={performances} />
        ) : latestConfirmed ? (
          <LatestResult perf={latestConfirmed} />
        ) : (
      ...
      {/* Bottom ranking strip (when showing current/latest) */}
